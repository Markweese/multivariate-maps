<template>
  <div class='datafilter'>
    ...
    <div class='datafilter__right'>
      <button class="button button-blue button-small --hollow --no-mobile" type="button" name="button" v-bind:aria-label="showAriaTable ? 'Hide ARIA table' : 'Show ARIA table'" v-on:click="toggleTable($event)" tabindex='0'>
        {{showAriaTable ? 'Hide' : 'Show'}} ARIA Table
      </button>
      <button v-on:click='toggleFilter()' v-on:keyup='processKeyEvent($event)' class='button button-blue button-small --hollow datafilter__right--filter' aria-role='dropdown' aria-label='time filter' v-bind:aria-expanded='filterOpen ? true : false' tabindex='0'>
